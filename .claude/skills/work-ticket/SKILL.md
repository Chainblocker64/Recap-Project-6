...
3. Determine which skill to dispatch to next. The workflow's phases, in order, are currently: `refine` → `plan`. Each phase-skill writes its *own* name into `work/<number>/state.json` as its last step, once it has finished - so the recorded `phase` is the last *completed* phase, not a pending one.
